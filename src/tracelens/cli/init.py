...
test: it fails if the run itself crashes or if infrastructure errors prevent
evaluation (`fail_on_infra_errors: true` in `tracelens.yaml` catches 100%
infra errors, broken imports, and missing credentials), but does not block on
agent task score regressions until baselines are enabled.
...
# eval suite runs on every pull request, failing if the run errors or if
# infrastructure failures prevent evaluation (via fail_on_infra_errors).
