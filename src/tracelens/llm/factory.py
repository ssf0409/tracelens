...
tracelens ships the ``LLMProvider`` ABC and an ``InMemoryProvider`` for
...
tracelens decoupled from any specific LLM vendor and avoids shipping a
thin wrapper that duplicates what the vendor SDK already does well.

Canonical user pattern::

    from tracelens import LLMProvider
...
def create_provider(model_or_alias: str, **kwargs: Any) -> LLMProvider:
    ...
    Args:
        model_or_alias: Currently only ``"in-memory"`` is supported.
            ...
            tracelens no longer ships a built-in LiteLLM/OpenAI/Anthropic
    ...
    raise ValueError(
        ...
        "directly — tracelens no longer ships a built-in provider wrapper. "
        "See tracelens.llm.factory docstring for the canonical pattern."
    )
