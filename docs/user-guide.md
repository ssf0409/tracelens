...
TraceLens separates aggregate reporting from raw trial evidence:

- **Aggregate artifacts** (`results.json`, `report.md`, `report.html`, CI summaries):
  Contain high-level metrics, pass rates, bootstrap intervals, and regression gate
  decisions. They do not embed raw agent conversation history or full error payloads.
- **Raw evidence artifacts** (`trials.json`, `checkpoint.json`, `failures.json`/`failures.html`, and `sample` worksheets):
  Capture verbatim execution evidence — agent inputs, full transcripts, tool invocations,
  adapter exceptions, and grader feedback. These artifacts may contain sensitive operational
  data, API keys, credentials, or personal data emitted during execution.
  - Checkpoint files (`--checkpoint`) survive interrupted runs for resume but are
    automatically removed on a clean exit 0 unless `--keep-checkpoint` is passed.
  - Worksheets generated by `tracelens sample` support `--excerpt-field <key>` to excerpt only
    the specific evaluated dictionary key rather than full transcript dumps.
  - Explicit export redaction and scrubbing policies are tracked in issue
    [#106](https://github.com/ssf0409/tracelens/issues/106); until then, treat raw artifacts as
    confidential evidence and review them before sharing externally.
