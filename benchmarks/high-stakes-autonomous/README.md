...
> `skipped (no gradable trials)`, not as a SEVERE regression. Such a
> baseline-backed task makes the CLI gate `UNEVALUABLE` (exit 2): fix the
> infrastructure and rerun before treating the check as passed.
